...
def page_diagnosis():
    ...
    if st.session_state.phase == "setup":
        ...
        selected_dims = [k for k, v in DIMENSIONES.items() if st.checkbox(v)]
        
        if st.button("🚀 Iniciar conversación"):
            if not company or not selected_dims:
                st.error("Completá los campos obligatorios.")
            else:
                import google.generativeai as genai
                genai.configure(api_key=api_key)
                # Modelo optimizado para cuota según tu lista de modelos disponibles
                model = genai.GenerativeModel("gemini-3.1-flash-lite")
                chat = model.start_chat(history=[])
                
                greeting = llamar_gemini(chat, f"Hola, empresa: {company}. Iniciá la entrevista.", es_chat=True)
                if greeting:
